...
const followerController = {
  async follow(req, res, next) {
    const user = req.user;
    const followingId = req.params.followingUserName;

    const searchedUser = await User.findOne({ username: followingId });
    if (!searchedUser) {
  ...
  async unfollowUsername(req, res, next) {
    const user = req.user;
    const unfollowingId = req.params.unfollowingUsername;

    try {
      // Find the user to unfollow
      const searchedUser = await User.findOne({ username: unfollowingId });
  ...
  async sendFriendRequest(req, res, next) {
    ...
    const searchedUser = await User.findOne({ username: friendId });
    if (!searchedUser) {
  ...
  async acceptFriendRequest(req, res, next) {
    ...
    try {
      ...
      if (user.pendingFriends.includes(friendId)) {
        ...
        user.pendingFriends = user.pendingFriends.filter((friend) => friend.toString() !== friendId);
  ...
  async getAllUsers(req, res, next) {
    try {
      ...
      const users = await User.find({ _id: { $ne: excludeUserId } }).select("_id firstName lastName username profilePicture followers friends pendingFriends");
